    ...
    def __split_sequence_disent(self):
        ...
        for item_seq in self.user_seq:
            if len(item_seq) == 0:
                print("got empty seq:")
                continue
            if len(item_seq) - 2 <= self.max_len:
                # self.part_sequence.append(item_seq[-(self.max_len + 2):-2])
                self.part_sequence.append(item_seq[:-2])
            else:
                # self.part_sequence.append(item_seq[-(self.max_len + 2):-2])
                beg_idx = list(range(len(item_seq) - 2 - self.max_len, 0, -sliding_step))
                beg_idx.append(0)
                for i in beg_idx[::-1]:
                    self.part_sequence.append(item_seq[i:i + self.max_len])  # keeping same as train set
    ...
    def _get_input_label_subsequences(seq_len: int,
        ...
        if seq_len == 2:
            ...
            if not seq_len == 1:
                ...
                # t = seq_len - 1 # temporarily for SASRec
        # input sub-sequence
        if seq_len == 1:
            inp_subseq = sequence
        else:
...
class FinetuneDataset(Dataset):
    ...
    def __init__(self, args, user_seq, item_frequency, test_neg_items=None, data_type='train'):
        self.args = args
        self.user_seq = user_seq
        self.test_neg_items = test_neg_items
        self.data_type = data_type
        self.max_len = args.max_seq_length
        self.item_frequency = item_frequency / sum(item_frequency)
    ...
    def __getitem__(self,
        ...
        target_neg = []
        seq_set = set(items)
        item_set = np.array(list(set(np.arange(self.args.item_size)) - seq_set))
        item_frequency = self.item_frequency[item_set]
        item_frequency = item_frequency / sum(item_frequency)
        # for _ in range(100):
        #     target_neg.append(neg_sample(seq_set, self.args.item_size, self.item_frequency))
        neg_samp= neg_sample(item_set, self.args.item_size, item_frequency)
        target_neg = list(neg_samp)
